...
class AttUNetBlockWithSelfAttention(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        self.AttUNet_block = AttUNetBlock(
    ...
    def forward(self, x: Tensor, noise_level: Tensor) -> Tensor:
        return self.self_attention(self.AttUNet_block(x, noise_level))
...
class AttUNet(nn.Module):
    def __init__(self, config: AttUNetConfig) -> None:
        super().__init__()
    ...
    def forward(self, x: Tensor, noise_level: Tensor) -> Tensor:
        ...
        for block in self.top_encoder_blocks:
            if isinstance(block, AttUNetBlock):
        ...
        for block in self.mid_encoder_blocks:
            if isinstance(block, AttUNetBlockWithSelfAttention):
                h = block(h, noise_level)
    ...
    def _make_mid_block(
        ...
        out_channels: int,
        dropout: float,
    ) -> AttUNetBlockWithSelfAttention:
        return AttUNetBlockWithSelfAttention(
            in_channels,
            out_channels,
            self.config.noise_level_channels,
            self.config.n_heads,
    ...
    @classmethod
    def from_pretrained(cls, pretrained_path: str) -> "AttUNet":
        with open(os.path.join(pretrained_path, "config.json"), mode="r") as f:
            config_dict = json.load(f)
        config = AttUNetConfig(**config_dict)
